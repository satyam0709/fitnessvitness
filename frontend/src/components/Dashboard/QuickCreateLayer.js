...
export function QuickTaskModal({ open, onClose }) {
  ...
  async function onSubmit(e) {
    ...
    const due = toSqlDateTime(endDt || startDt);
    let bodyDesc = description.trim();
    if (startDt && endDt && startDt !== endDt) {
      bodyDesc = `Start: ${toSqlDateTime(startDt)}\n${bodyDesc}`;
    }
    setSaving(true);
    try {
      const res = await apiFetch("/tasks", {
        ...
        body: JSON.stringify({
          ...
          description: bodyDesc || null,
          assigned_to: assignedTo ? Number(assignedTo) : null,
          due_date: due,
          priority,
          status: taskStatusForDb(status),
        }),
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok || !json.success) {
        setErr(json.message || "Could not create task");
        return;
      }
  ...
  return (
    <CrmShellModal open={open} title="Add Task" onClose={onClose} wide>
      <form
      ...
      >
        <div className={styles.body}>
          ...
          <div className={styles.field}>
            <label className={styles.label} htmlFor={`${id}-sub`}>
          ...
          <div className={styles.row3}>
            <div className={styles.field}>
              <label className={styles.label} htmlFor={`${id}-pri`}>Priority</label>
              <select
                id={`${id}-pri`}
            ...
            <div className={styles.field}>
              <label className={styles.label} htmlFor={`${id}-st`}>Status</label>
              <select
                id={`${id}-st`}
                className={styles.select}
                value={status}
              ...
              >
                ...
                <option value="in_feedback">In Feedback</option>
            ...
            <div className={styles.field}>
              <label className={styles.label} htmlFor={`${id}-lbl`}>Label (Optional)</label>
              <input
                id={`${id}-lbl`}
                className={styles.input}
                value={label}
          ...
          <div className={styles.toggleRow}>
            <span className={styles.label}>Recursive</span>
            <button
              type="button"
              className={`${styles.switch} ${recursive ? styles.switchOn : ""}`}
          ...
          <p className={styles.hint}>Recurrence is visual only until the API supports it.</p>
          <div className={styles.row2}>
            <div className={styles.field}>
              <label className={styles.label} htmlFor={`${id}-start`}>Start date</label>
              <input
                id={`${id}-start`}
                type="datetime-local"
                className={styles.input}
            ...
            <div className={styles.field}>
              <label className={styles.label} htmlFor={`${id}-end`}>End date (due)</label>
              <input
                id={`${id}-end`}
                type="datetime-local"
                className={styles.input}
          ...
          <div className={styles.field}>
            <label className={styles.label} htmlFor={`${id}-asg`}>
              Assign To <span className={styles.hint}>(optional)</span>
            </label>
            <select
              id={`${id}-asg`}
              className={styles.select}
          ...
          <div className={styles.field}>
            <label className={styles.label} htmlFor={`${id}-desc`}>Description</label>
            <textarea
              id={`${id}-desc`}
