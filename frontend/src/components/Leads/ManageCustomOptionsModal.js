...
import styles from "./ManageCustomOptionsModal.module.css";

const TABS = [
  { key: "source", label: "Sources", singular: "Source" },
  { key: "label", label: "Labels", singular: "Label" },
...
/**
 * Manage Sources / Labels / Statuses custom options (backend registry).
 * Rename + cascade-delete. Creation happens via lead form "Other".
 */
export default function ManageCustomOptionsModal({ onClose, onDone }) {
  ...
  async function handleRename(fieldName, oldValue) {
    ...
    setErr("");
    try {
      const res = await apiFetch("/leads/custom-options/rename", {
        method: "PUT",
  ...
  async function handleDelete(fieldName, optionValue) {
    ...
    try {
      ...
      showToast(`Deleted "${optionValue}" and matching leads`);
      setDeleteTarget(null);
      await fetchOptions();
      onDone?.();
    } catch {
        ...
        <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
          ...
          <div className={styles.body}>
            <p className={styles.hint}>
              All custom values manually entered by users are listed below. Standard defaults are
              managed system-wide.
            </p>
            ...
            {loading ? (
              ...
              <div className={styles.empty}>
                ...
                <small>Create one by picking &quot;Other&quot; when saving a lead.</small>
              </div>
            ) : (
