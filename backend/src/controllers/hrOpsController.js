...
async function getPayroll(req, res) {
  try {
    ...
    let query = "SELECT * FROM hr_payroll WHERE tenant_id = ?";
...
async function upsertPayroll(req, res) {
  try {
    ...
    const [existing] = await pool.execute(
      "SELECT id FROM hr_payroll WHERE tenant_id = ? AND user_id = ? AND month = ? AND year = ?",
    ...
    const [result] = await pool.execute(
      `INSERT INTO hr_payroll (tenant_id, user_id, month, year, salary, bonuses, deductions, net_pay, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [tenantId, userId, month, year, salary || 0, bonuses || 0, deductions || 0, netPay || 0, notes || ""]
    );
...
async function markPayrollPaid(req, res) {
  try {
    ...
    const { tenantId } = req;
    await pool.execute(
      "UPDATE hr_payroll SET status = 'paid', paid_at = NOW(), updated_at = NOW() WHERE id = ? AND tenant_id = ?",
      [payrollId, tenantId]
    );
...
async function getAppraisals(req, res) {
  try {
    ...
    let query = "SELECT * FROM hr_appraisals WHERE tenant_id = ?";
    const params = [tenantId];
    if (userId) { query += " AND user_id = ?"; params.push(userId); }
...
async function createAppraisal(req, res) {
  try {
    ...
    const [result] = await pool.execute(
      `INSERT INTO hr_appraisals (tenant_id, user_id, year, rating, strengths, improvements, comments)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [tenantId, userId, year, rating || 0, strengths || "", improvements || "", comments || ""]
    );
    res.status(201).json({ success: true, id: result.insertId });
  } catch (err) {
